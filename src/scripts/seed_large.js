...
const TARGET_USER_EMAIL = 'phcunguyen@gmail.com';
...
const seedLargeData = async () => {
  try {
    ...
    logger.info('Cleared existing quizzes, questions, attempts, and statistics.');
    ...
    const topics = ['Daily Life', 'Work', 'Travel', 'Education', 'Food', 'Hobbies', 'Business', 'Health', 'Culture', 'Shopping'];
    ...
    for (let i = 0; i < insertedQuizzes.length; i++) {
        const quiz = insertedQuizzes[i];
        const startIdx = i * 10;
        quiz.questionIds = insertedQuestions.slice(startIdx, startIdx + 10).map(q => q._id);
        await quiz.save();
    }
